...
export function FloatingVoiceRecorder({ strandId, planTier, onTranscriptReady }: FloatingVoiceRecorderProps) {
  ...
  const uploadRecording = useCallback(async () => {
    ...
    try {
      ...
      if (attachTranscript && onTranscriptReady && payload?.data?.transcriptText) {
        onTranscriptReady(payload.data.transcriptText);
      }
    } catch (error) {
      console.error(error);
      setState('error');
  ...
  }, [audioBlob, strandId, attachTranscript, generateSummary, selectedLanguage, elapsed, onTranscriptReady]);
              ...
              <div className="space-y-3 rounded-md bg-muted/40 p-3">
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Slider
                    ...
                    defaultValue={[0]}
                    onValueChange={(value) => {
                      const options = ['auto', 'en', 'es'];
                      setSelectedLanguage(options[value[0]] ?? 'auto');
                    }}
                  />
                </div>
