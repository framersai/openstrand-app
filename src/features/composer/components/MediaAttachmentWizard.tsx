 ...
 *
 * Allows users to attach images or videos to a strand with optional AI summary and OCR.
 * - Supports file picker, drag-and-drop, and device camera capture (Capacitor).
 * - Optionally runs OCR (handwriting/print) and selects OCR language.
 * - On success, invokes `onUploaded` with the server response (including signed URL).
 */

import { useCallback, useMemo, useState } from 'react';
import { AnimatePresence, motion } from 'framer-motion';
import { Camera, ChevronLeft, ChevronRight, Film, Info, UploadCloud, ScanText } from 'lucide-react';
...
export function MediaAttachmentWizard({ strandId, planTier, onUploaded }: MediaAttachmentWizardProps) {
  ...
  const captureWithCamera = useCallback(async () => {
    try {
      if (Capacitor.isNativePlatform()) {
        ...
        setMediaType('image');
        setFile(f);
        await platformStorage.saveMedia(`camera-${Date.now()}`, blob, 'image').catch(() => {});
      } else {
        // Fallback: hint mobile browsers to open camera
  ...
  const upload = useCallback(async () => {
    ...
    try {
      ...
      if (runOCR) {
        ...
        if (ocrLanguage && ocrLanguage !== 'auto') {
  ...
  }, [file, generateSummary, mediaType, notes, reset, strandId, runOCR, ocrLanguage]);
              ...
              {step === 0 && (
                ...
                >
                  <div className="flex flex-col gap-2">
                    ...
                    <div
                      className={"flex items-center gap-2 rounded-md border border-dashed p-2 " + (dragOver ? 'bg-muted/40' : '')}
                    ...
                    >
                      ...
                      <Button type="button" variant="ghost" size="icon" onClick={captureWithCamera}>
                        <Camera className="h-4 w-4" />
                      </Button>
                    </div>
                  </div>
              ...
              {step === 1 && (
                ...
                >
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Switch id="run-ocr" checked={runOCR} onCheckedChange={setRunOCR} />
                      <Label htmlFor="run-ocr" className="text-xs flex items-center gap-1"><ScanText className="h-3.5 w-3.5" /> Run OCR (handwriting)</Label>
                    </div>
                    {runOCR ? (
                      <div className="flex items-center gap-2">
