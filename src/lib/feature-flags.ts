...
export function useFeatureFlags() {
  ...
  const features = useMemo(() => {
    ...
    if (typeof capabilities?.aiArtisan === 'boolean') {
      overrides.aiArtisan = capabilities.aiArtisan;
    }

    if (Array.isArray(capabilities?.storage?.capabilities)) {
      overrides.storageAdapters = capabilities.storage.capabilities.length > 0;
    }

    if (typeof capabilities?.storage?.syncEnabled === 'boolean') {
      overrides.supabaseSync = base.supabaseSync || capabilities.storage.syncEnabled;
    }
    ...
    variant,
    resolvedPlan,
    capabilities?.aiArtisan,
    capabilities?.storage?.capabilities,
    capabilities?.storage?.syncEnabled,
  ]);
