...
export const strandAPI = {
  ...
  /**
   * Partially update only the strand metadata (merge semantics)
   */
  async updateMetadata(
    id: string,
    metadata: Partial<StrandMetadata> | Record<string, unknown>,
  ): Promise<Strand> {
    const response = await apiFetch(`/strands/${id}/metadata`, {
      method: 'PATCH',
...
export const openstrandAPI = {
  ...
  attachments: {
    /**
     * List attachments globally or by strand
     */
    async list(filters?: { strandId?: string; type?: 'image' | 'video' | 'audio'; q?: string; mine?: boolean; page?: number; pageSize?: number }): Promise<any[]> {
      const params = new URLSearchParams();
      if (filters?.strandId) params.set('strandId', filters.strandId);
      ...
      const query = params.toString();
      const response = await apiFetch(query ? `/attachments?${query}` : '/attachments');
      return await parseData<any[]>(response);
    },
    ...
    async uploadMedia(
      ...
      options?: { generateSummary?: boolean; runOCR?: boolean; ocrLanguage?: string }
    ): Promise<any> {
      const form = new FormData();
      form.append('file', file);
      form.append('type', type);
      if (options?.generateSummary !== undefined) form.append('generateSummary', String(options.generateSummary));
      if (options?.runOCR !== undefined) form.append('runOCR', String(options.runOCR));
      if (options?.ocrLanguage) form.append('ocrLanguage', options.ocrLanguage);
      const response = await apiFetch(`/strands/${strandId}/attachments/media`, {
        method: 'POST',
        body: form,
      });
      return await parseData(response);
    ...
    /**
     * Update attachment label/summary/tags/metadata
     */
    async update(
      attachmentId: string,
      updates: { summary?: string; label?: string; tags?: string[]; metadata?: Record<string, unknown> }
    ): Promise<any> {
  ...
  billing: {
    ...
    async usageSummary(options?: { scope?: 'user' | 'team'; teamId?: string; start?: string; end?: string }): Promise<any> {
      const params = new URLSearchParams();
      ...
      const query = params.toString();
      const response = await apiFetch(query ? `/billing/usage?${query}` : '/billing/usage');
      return await parseData(response);
    },
    ...
    async portal(returnUrl: string): Promise<{ portalUrl: string }> {
      const response = await apiFetch(`/billing/portal?returnUrl=${encodeURIComponent(returnUrl)}`);
      return await parseData(response);
    },
  },

  aiProviders: {
    async list(): Promise<{ systemProviders: string[]; userProviders: string[]; hasGlobalOpenAI: boolean; hasGlobalAnthropic: boolean; hasUserOpenAI: boolean; hasUserAnthropic: boolean; }> {
