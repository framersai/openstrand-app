...
const createOfflineCapabilities = (): CapabilityMatrix => ({
  ...
  environment: {
    mode: 'offline',
    auth: 'local',
    storagePath: 'sqlite:./openstrand.db',
  },
  storage: {
    kind: 'sqlite',
    path: 'sqlite:./openstrand.db',
    writable: true,
    capabilities: ['persistence', 'transactions'],
    syncEnabled: false,
  },
  notes: [
    'Running in local-first mode. Everything stays on this device.',
    'Set OPENSTRAND_SYNC_REMOTE_URL to enable automatic cloud backups.',
  ],
  local: {
      ...
      (set, get) => ({
        ...
        loadCapabilities: async () => {
          ...
          try {
            ...
            const storageSource = capabilities?.storage as {
              kind?: string;
              driver?: string;
              path?: string;
              writable?: boolean;
            ...
            const storageCapabilities = Array.isArray(storageSource?.capabilities)
              ? [...storageSource!.capabilities]
              : undefined;

            const storageWritable =
              typeof storageSource?.writable === 'boolean'
                ? storageSource.writable
                : storageCapabilities?.includes('persistence') ?? true;

            const storageKind = storageSource?.kind ?? storageSource?.driver ?? 'unknown';
            const normalized: CapabilityMatrix = {
              ...
              storage: storageSource
                ? {
                    kind: storageKind,
                    driver: storageSource.driver ?? storageKind,
                    path: storageSource.path,
                    writable: storageWritable,
                    capabilities: storageCapabilities,
                    syncEnabled:
                      typeof storageSource.syncEnabled === 'boolean'
                        ? storageSource.syncEnabled
                        : undefined,
                  }
                : undefined,
              notes: Array.isArray(capabilities?.notes) ? [...capabilities.notes] : undefined,
              local: {
                onboardingComplete:
